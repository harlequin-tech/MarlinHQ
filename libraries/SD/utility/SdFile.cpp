 ...
 */
int8_t SdFile::readDir(dir_t* dir, char *longFilename) 
{
    int8_t n;
	...
	if (DIR_IS_LONG_NAME(dir) && longFilename != NULL) {
		 ...
		 * And the sequence number should be higher then 0
		 */
	...
	if (DIR_IS_FILE_OR_SUBDIR(dir)) {
	    if (!gotLongName) {
		/* Return the short name in the long file name string */
		for (uint8_t ind=0; ind<sizeof(dir->name); ind++) {
		    if (dir->name[ind] == ' ') continue;
		    if (ind == 8) {
			*longFilename++ = '.';
		    }
		    *longFilename++  = dir->name[ind];
		}
		*longFilename = 0;
	    }
	    return n;
